...
# a_weights =  tf.Variable(tf.truncated_normal([num_tags+1, embedding_size],stddev=1.0 / math.sqrt(embedding_size)))
...
with tf.name_scope("loss") as scope:
    ...
    # add hide level
    # node_embed = tag_embed1*a_weights*tag_embed1
    # Compute the softmax loss, using a sample of the negative labels each time.   

    train_context_node_flat_float = tf.to_float(train_context_node_flat)
    loss_node2vec = tf.reduce_mean(tf.nn.sampled_softmax_loss(softmax_weights,softmax_biases,train_context_node_flat_float,node_embed,num_sampled, num_nodes))
...
with tf.Session() as sess:
    ...
    sess.run(init)
    for i in range(0,num_random_walks):
        a_random_walk=np_random_walks[i]
        train_input_batch = np.array([a_random_walk[j] for j in range(random_walk_length-context_size)])
        train_context_batch = np.array([a_random_walk[j+1:j+1+context_size] for j in range(random_walk_length-context_size)])
        ...
        test  = sess.run([update_loss,loss_node2vec,merged],feed_dict)
